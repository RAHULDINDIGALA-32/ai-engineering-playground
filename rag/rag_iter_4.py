...
with open("knowledge_base.json", "r", encoding="utf-8") as f:
    documents = json.load(f).get("policies")
...
document_texts = [document["description"] for document in documents]
...
# Search Qdrant Collection with appropriate filters
def search_db_with_filter(query, query_filter=None, top_k=3):
...
sample_query = "When can an employee claim home internet reimbursement?"
#results = search_db(sample_query, top_k=3)
results = search_db_with_filter(sample_query, benefits_filter, top_k=5)
...
def rag_pipeline(query):
    ...
    results = search_db_with_filter(query, benefits_filter, top_k=5)
